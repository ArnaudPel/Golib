...
class Collection:
    ...
    def my_start_gametree(self):
        self.children.append(createtree(self, self.parser))
...
class GameTree:
    ...
    def my_start_node(self):
        ...
        elif isinstance(self.parent, GameTree):
            previous = self.parent.nodes[-1]
        else:
            previous = None
        node = createnode(self, previous, self.parser)
        if len(self.nodes) == 0:
            # node.first = 1
            if isinstance(self.parent, GameTree):
...
def createtree(parent, parser=None):
    """ Had to modify the original code at least to that extent to force extensibility. """
    return GameTree(parent, parser=parser)


def createnode(parent, previous, parser=None):
    """ Had to modify the original code at least to that extent to force extensibility. """
    return Node(parent, previous, parser=parser)
